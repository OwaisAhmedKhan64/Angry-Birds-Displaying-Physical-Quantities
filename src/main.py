...
def sling_action():
    ...
    #bird = Bird(mouse_distance, angle, 154, 156, space)
    #velocity = math.sqrt((bird.body.velocity.x) ** 2 + (bird.body.velocity.y) ** 2)
...
# bird and pigs
space.on_collision(0, 1 , None , None , post_solve=post_solve_bird_pig)#.post_solve=post_solve_bird_pig
# bird and wood
space.on_collision(0, 2 , None , None , post_solve=post_solve_bird_wood)#.post_solve=post_solve_bird_wood
# pig and wood
space.on_collision(1, 2 , None , None , post_solve=post_solve_pig_wood)#.post_solve=post_solve_pig_wood
load_music()
...
while running:
    ...
    for pig in pigs:
        ...
        angle_surface = font.render(f"Angle: {-angle_bird_degrees:.1f}°", True, (0, 0, 0))
        angle_rect = angle_surface.get_rect()
        angle_rect.center = 200, 200
        screen.blit(angle_surface, angle_rect)
        if mouse_distance > rope_lenght:
            mouse_distance = rope_lenght
        velocity = (mouse_distance * 53) / 5
        velocity_surface = font.render(f"Velocity: {velocity:.1f} pix/s" , True , (0,0,0))
        velocity_rect = velocity_surface.get_rect()
        velocity_rect.center = 200 , 230
        screen.blit(velocity_surface , velocity_rect)
